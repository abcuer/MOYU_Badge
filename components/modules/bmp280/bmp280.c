...
/**
 * @brief 设置 BMP280 的电源模�?
 * @param enable true 为进�?Sleep 休眠, false 为恢�?Normal 测量
 */
...
void bmp280_init(void) 
{
    ...
    // 3. 配置 BMP280 (�?0xF4 设置�?Normal Mode)
...
static float bmp280_get_altitude(float pressure_hpa)
{
    float p0 = (weather_data.sea_level_hpa > 900.0f)
            ? weather_data.sea_level_hpa   // 有网络数据用实时�?
            : 1013.25f;                     // 没有则用标准�?
