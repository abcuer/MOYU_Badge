...
void BloodDataUpdate(void) {
    ...
    while(i < FFT_N) {
        max30102_read_fifo();
        if(fifo_ir > 10000) { 
            s1[i].real = (float)fifo_red;
            s1[i].imag = 0;
            s2[i].real = (float)fifo_ir;
            s2[i].imag = 0;
            i++;
            // 确保这里使用的是 FreeRTOS 的延迟，出让 CPU 权限
            vTaskDelay(pdMS_TO_TICKS(10)); 
        } else {
            b_data.heart = 0;
            b_data.SpO2 = 0;
            vTaskDelay(pdMS_TO_TICKS(100)); // 没放手时也稍微歇一下
            return;
        }
    }
}
...
void BloodDataTranslate(void) 
{
    ...
    // 2. 减去直流分量以获取交流信号 (AC)，并执行汉宁窗（可选，增加频率准确度）
    for(i = 0; i < FFT_N; i++) {
        s1[i].real -= dc_red;
        s1[i].imag = 0;
        s2[i].real -= dc_ir;
    ...
    // 5. 计算心率 (Heart Rate)
    // 寻找红外光谱中峰值最大的索引（心率在红外信号上更明显）
    int max_index = find_max_num_index(s2, FFT_N / 2); 
    
    // 频率换算：Fs = 100Hz (10ms采样), FFT_N = 512
    // 心率 = 峰值索引 * (Fs / FFT_N) * 60 (bpm)
    if (max_index >= START_INDEX) {
        b_data.heart = (int)(max_index * (100.0f / FFT_N) * 60);
    } else {
        b_data.heart = 0;
    }

    // 6. 计算血氧饱和度 (SpO2)
    // 根据公式：R = (AC_red / DC_red) / (AC_ir / DC_ir)
    if (dc_red > 0 && dc_ir > 0 && ac_ir > 0) {
        float R = (ac_red / dc_red) / (ac_ir / dc_ir);
        
        // 血氧经验公式：SpO2 = -45.06 * R * R + 30.354 * R + 94.845
        // 或者简化线性公式：SpO2 = 110 - 25 * R
        float spo2_val = 110.0f - 25.0f * R;

        // 范围限幅
        if (spo2_val > 100.0f) spo2_val = 100.0f;
        if (spo2_val < 60.0f)  spo2_val = 0.0f;
        
        b_data.SpO2 = spo2_val;
    } else {
        b_data.SpO2 = 0;
    }
}
